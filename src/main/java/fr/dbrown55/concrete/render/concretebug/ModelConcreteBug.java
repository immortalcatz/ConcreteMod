...
public class ModelConcreteBug extends ModelBase {
	...
	public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
		if(entityIn instanceof EntityConcreteBug && ((EntityConcreteBug)entityIn).isSolid()){
			for(int i = 0; i < this.silverfishBodyParts.length; ++i) {
				this.silverfishBodyParts[i].rotateAngleY = 0.0F;
				this.silverfishBodyParts[i].rotationPointX = 0.0F;
			}
		} else {
			for(int i = 0; i < this.silverfishBodyParts.length; ++i) {
