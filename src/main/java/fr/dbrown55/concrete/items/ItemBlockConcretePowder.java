...
public class ItemBlockConcretePowder extends DbrownItemBlock {
	...
	public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if(this.base == BlockHandler.VANILLA_POWDER && worldIn.getBlockState(pos).getBlock() == Blocks.MONSTER_EGG && !worldIn.isRemote) {
			...
			bug.setPositionAndRotation(pos.getX(), pos.getY(), pos.getZ(), (float) (MathHelper.clamp_double(MathHelper.floor_double((double)(playerIn.rotationYaw * 4.0F / 360.0F) + 0.5D) - 180.0, 0.0, 360.0)), 0.0f);
