...
public class ItemBlockGate extends DbrownItem {
	
	private ConcreteGateBase[] blockList;
	
	public ItemBlockGate(ConcreteGateBase[] blocks, ResourceLocation name) {
		super(name);
	...
	public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		ConcreteGateBase toPlace = null;
		for(ConcreteGateBase placable : this.blockList) {
			if(placable.isCorrectGate(stack)) {
				toPlace = placable;
			}
		}

		if(toPlace != null && stack.stackSize != 0 && playerIn.canPlayerEdit(pos, facing, stack) && worldIn.canBlockBePlaced(toPlace, pos, false, facing, (Entity)null, stack)) {
			int i = this.getMetadata(stack.getMetadata());
			IBlockState iblockstate1 = toPlace.getStateForPlacement(worldIn, pos, facing, hitX, hitY, hitZ, i, playerIn, stack);
