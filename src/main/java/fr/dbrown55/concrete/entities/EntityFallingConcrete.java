...
public class EntityFallingConcrete extends EntityFallingBlock {
	...
	public void onUpdate() {
		...
		if(this.worldObj.getBlockState(this.getPosition()).getMaterial() == Material.WATER
			|| this.worldObj.getBlockState(this.getPosition().north()).getMaterial() == Material.WATER
			|| this.worldObj.getBlockState(this.getPosition().south()).getMaterial() == Material.WATER
			|| this.worldObj.getBlockState(this.getPosition().east()).getMaterial() == Material.WATER
			|| this.worldObj.getBlockState(this.getPosition().west()).getMaterial() == Material.WATER){
			...
			if(color != null){
				...
				this.worldObj.setBlockState(this.getPosition(), state);
			}
			this.setDead();
		}
	}
