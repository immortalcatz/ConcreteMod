...
public class EntityConcreteBug extends EntityMob {
	...
	public static void registerFixesConcreteBug(DataFixer fixer) {
		EntityLiving.registerFixesMob(fixer, EntityConcreteBug.class);
	}
	...
	public void onEntityUpdate() {
		...
		if(!this.world.isRemote) {
			if(this.getEntityBoundingBox() != null && this.world.isMaterialInBB(this.getEntityBoundingBox(), Material.WATER) && !this.isSolid()) {
				this.setSolid(true);
			}
		}
	}
	...
	protected boolean processInteract(EntityPlayer player, EnumHand hand) {
		...
		if(hand == EnumHand.MAIN_HAND && stack == null && this.isSolid() && !this.world.isRemote) {
			ItemStack is = new ItemStack(ItemHandler.CONCRETE_BUG);
			NBTTagCompound nbt = new NBTTagCompound();
			nbt.setInteger("color", this.getColor());
			is.setTagCompound(nbt);
			EntityItem item = new EntityItem(this.world, this.posX, this.posY, this.posZ, is);
